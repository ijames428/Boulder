...
public:
	...
	float FastCosineTable[MAX_CIRCLE_ANGLE];
	...
	inline float fastcos(float n) {
		...
		return(
			i < 0 ?
			FastCosineTable[((-i) + QUARTER_MAX_CIRCLE_ANGLE) & MASK_MAX_CIRCLE_ANGLE] :
			FastCosineTable[(i + QUARTER_MAX_CIRCLE_ANGLE) & MASK_MAX_CIRCLE_ANGLE]);
	}
	...
	inline float fastsin(float n) {
		...
		return(
			i < 0 ?
			FastCosineTable[(-((-i)&MASK_MAX_CIRCLE_ANGLE)) + MAX_CIRCLE_ANGLE] :
			FastCosineTable[i & MASK_MAX_CIRCLE_ANGLE]);
	}
  ...
  LightSystem() {
    DrawFont.loadFromFile("Images/Shaders/ABSTRACT.TTF");
    ...
    for (int i = 0; i < MAX_CIRCLE_ANGLE; ++i) {
      FastCosineTable[i] = (float)( std::sin((double)i * PI / HALF_MAX_CIRCLE_ANGLE) );
    }
